...
public class TwinkleTwinkleWithChordsAndDrums {
  public static void main(String[] args) throws MidiUnavailableException, InterruptedException {
    ...
    for (int i = 0; i < melodyNotes.length; i++) {
      ...
      for (int j = 0; j < chordNotes.length; j++) {
        chordChannel.noteOn(chordNotes[j], 50); // 将和弦音量值改为50
      }
    ...
    new Thread(() -> {
      try {
        ...
        drumsChannel.noteOn(42, 100);
        Thread.sleep(1000);
        drumsChannel.noteOff(42);
        drumsChannel.noteOff(38);
        drumsChannel.noteOff(46);
